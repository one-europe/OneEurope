...
class RSS extends Plugin {
	...
	public function filter_rewrite_rules( $rules )
	{
		...
		$rules[] = new RewriteRule( array( 
			'name' => 'rss_feed_debate',
			'parse_regex' => '#^debates/(?P<slug>[^/]+)/rss(?:/page/(?P<page>\d+))?/?$#i',
			'build_str' => 'debates/{$slug}/rss(/page/{$page})',
			'handler' => 'UserThemeHandler',
			'action' => 'rss_debate_collection',
			'priority' => 8,
			'is_active' => 1,
			'description' => 'RSS Debate Collection' )
		);
		// authors!
		$rules[] = new RewriteRule( array( 
			...
			'priority' => 8,
			'is_active' => 1,
			'description' => 'RSS Profile Collection' )
		);
	...
	public function create_rss_wrapper()
	{
		...
		$lastBuildDate = $channel->addChild( 'lastBuildDate', Post::get()->pubdate->format( DATE_RSS ) );
		$ttl = $channel->addChild( 'ttl', '60' );
	...
	/**
	 * Respond to requests for the RSS feed for a specific debate
	 * @param array $vars Handler variables as passed in by rewrite rules
	 */
	public function action_handler_rss_debate_collection($vars)
	{
		$slug = $vars['slug'];
	...
	/**
	 * Respond to requests for the RSS feed for a specific profile/author
	 * @param array $vars Handler variables as passed in by rewrite rules
	 */
	public function action_handler_rss_profile_collection($vars)
	{
		$slug = $vars['slug'];
	...
	public function theme_feed_rss_alternate( $theme )
	{
		...
		switch ( $rulename ) {
			...
			case 'display_entries_by_tag':
				...
				break;
			case 'display_debate':
				return URL::get( 'rss_feed_debate', array( 'slug' => Controller::get_var( 'slug' ) ) );
				break;
			case 'display_profile':
				return URL::get( 'rss_feed_profile', array( 'slug' => Controller::get_var( 'slug' ) ) );
				break;
			case 'display_home':
			default:
