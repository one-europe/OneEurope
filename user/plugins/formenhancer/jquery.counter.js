		...
		counter : function(options) {
			...
			options = $.extend({}, defaults, options), methods = {
				...
				if(options.warning >= 0){
						var counterWarning = 'warning';
					} else {
						var counterWarning == 0;
					} */


					// Insert counter after text area/box
					$('<div/>').attr('class', 'counter').html("<span id=" + counterID + "/> " + methods.setMsg()).insertAfter($obj);
				...
				/* Checks to see if the warning number has been reached */
				isWarningReached : function(e, _warning) {
					if(noLimit) {
						return false;
					}
					// Counting down
					if(options.count === defaults.count) {
						_warning = 0;
						return (countIndex <= _warning ) ? true : false;
