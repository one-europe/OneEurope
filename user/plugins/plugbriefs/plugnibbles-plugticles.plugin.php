...
class PlugNibble extends Plugin
{ 
	...
	public function help() {
		...
		$str.= '<p>Your theme needs to have a <code>nibble.single.php</code> template. If it does not, you can usually copy <code>plugnibble.single.php</code> to <code>nibble.single.php</code> in your file and use it.</p>';
		return $str;
	}
	...
	{ 
		$names = array( 
			self::CONTENT_TYPE => array(
				'singular' => _t( 'NibbleB', self::CONTENT_TYPE ),
				'plural' => _t( 'NibblesB', self::CONTENT_TYPE ),
			)
		); 
	...
	{
		// only edit the form if it's a nibble
		if ( $form->content_type->value == Post::type( self::CONTENT_TYPE ) ) {
			...
			//$form->append('text', 'append', 'null:null', _t('Append this nibble to the institution profile of ... (make sure that the name is spelled correctly!)'), 'admincontrol_text');
