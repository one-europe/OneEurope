				...
				<aside>
					...
					<?php } else {
							...
							?>
								...
								<div class="affiliated-posts tile-thumbs list-1 thumbs-list">
									<?php foreach ($pieces as $piece ) { ?>
										<?php if (!$piece->info->author || $piece->info->author == $post->info->user || $piece->info->author == 0 ) { ?>
										<div class="list">
											<div class="img-wrap">
												<img src="<?php echo $piece->info->photourl; ?>" alt="<?php if ( $piece->info->photoinfo ) { echo $piece->info->photoinfo; } else { echo $piece->title; } ?>" />
											</div>
											<header>
												<h2><a href="<?php echo $piece->permalink; ?>" title="<?php echo $piece->title; ?>"><?php echo $piece->title_out; ?></a></h2>
											</header>
											<article class="body"><?php if ( $piece->info->excerpt ) { echo $piece->info->excerpt; } else { echo $piece->content_out; } ?></article>
											<footer>
