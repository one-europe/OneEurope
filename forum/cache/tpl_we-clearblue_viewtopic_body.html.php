...
<h2>
	<a href="<?php echo (isset($this->_rootref['U_VIEW_TOPIC'])) ? $this->_rootref['U_VIEW_TOPIC'] : ''; ?>"><?php echo (isset($this->_rootref['TOPIC_TITLE'])) ? $this->_rootref['TOPIC_TITLE'] : ''; ?></a>&nbsp;&nbsp;<fb:like href="<?php echo (isset($this->_rootref['U_PAGE_URL'])) ? $this->_rootref['U_PAGE_URL'] : ''; ?>" show_faces="false" width="450" font="arial"></fb:like>
</h2>
...
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } ?>
	...
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } ?>">
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			...
			<dd><?php if ($_postrow_val['RANK_IMG']) {  echo $_postrow_val['RANK_IMG']; } else if ($_postrow_val['RANK_TITLE']) {  echo $_postrow_val['RANK_TITLE']; } ?></dd>
