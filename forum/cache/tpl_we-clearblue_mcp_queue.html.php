			...
			<table class="forums">
				<thead>
					<tr>
						...
						<th><?php if (! $this->_rootref['S_TOPICS']) {  echo ((isset($this->_rootref['L_TOPIC'])) ? $this->_rootref['L_TOPIC'] : ((isset($user->lang['TOPIC'])) ? $user->lang['TOPIC'] : '{ TOPIC }')); ?> &amp; <?php } echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); ?></th>
				...
				<tbody>
					<?php $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_DELETED_TOPIC']) {  ?>
							...
							<tr colspan="3" class="bg1">
								<td><p class="notopics"><?php echo ((isset($this->_rootref['L_DELETED_TOPIC'])) ? $this->_rootref['L_DELETED_TOPIC'] : ((isset($user->lang['DELETED_TOPIC'])) ? $user->lang['DELETED_TOPIC'] : '{ DELETED_TOPIC }')); ?></p></td>
							</tr>
						<?php } else { ?>

							<tr class="row<?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg2<?php } else { ?> bg1<?php } ?>">
								<td class="topic bg2">
									<h4><a href="<?php echo $_postrow_val['U_VIEW_DETAILS']; ?>" class="topictitle"><?php echo $_postrow_val['POST_SUBJECT']; ?></a></h4>
									<p><?php echo ((isset($this->_rootref['L_POSTED'])) ? $this->_rootref['L_POSTED'] : ((isset($user->lang['POSTED'])) ? $user->lang['POSTED'] : '{ POSTED }')); ?> <?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php echo $_postrow_val['POST_AUTHOR_FULL']; ?> &raquo; <?php echo $_postrow_val['POST_TIME']; ?></p>
								</td>
								<td>
									<p><?php if (! $this->_rootref['S_TOPICS']) {  echo ((isset($this->_rootref['L_TOPIC'])) ? $this->_rootref['L_TOPIC'] : ((isset($user->lang['TOPIC'])) ? $user->lang['TOPIC'] : '{ TOPIC }')); ?>: <a href="<?php echo $_postrow_val['U_TOPIC']; ?>"><?php echo $_postrow_val['TOPIC_TITLE']; ?></a><?php } ?></p>
									<p><?php echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); ?>: <a href="<?php echo $_postrow_val['U_VIEWFORUM']; ?>"><?php echo $_postrow_val['FORUM_NAME']; ?></a></p>
								</td>
								<td class="mark bg2"><input type="checkbox" name="post_id_list[]" value="<?php echo $_postrow_val['POST_ID']; ?>" /></td>
							</tr>
						<?php } }} ?>
